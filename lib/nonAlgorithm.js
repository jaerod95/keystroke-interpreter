module.exports = {

  /************************************************************************
   * Organizer for All parsing and link to csv conver and master analysis *
   * @param  {Array} data  An array of JSON KeyStroke Data Files          *
   * @return {void}       void;                                           *
   ************************************************************************/

    sort: (arr) => {
        arr.sort((a, b) => {
            return a.timeStamp - b.timeStamp;
        });
        return arr;
    },

  /************************************************************************
   * Organizer for All parsing and link to csv conver and master analysis *
   * @param  {Array} data  An array of JSON KeyStroke Data Files          *
   * @return {void}       void;                                           *
   ************************************************************************/

    key_count_of_type: (data, key) => {
        var result = { "total": 0 };
        for (pX in data) {                         //runs for each page eg.p1, p2, p3...
            result[pX] = { "total": 0 };
            var keys = data[pX].KeyEvents;
            keys.forEach((val, index, arr) => {     //runs for each data set on page. eg. p1-a, p1-b...
                result[pX][pX + '_' + index] = 0;
                val.forEach((v, i, a) => {
                    if (v.type == 'keydown' && v.which == key) {
                        result[pX].total += 1;
                        result[pX][pX + '_' + index] += 1;
                    }
                });
            });
            result.total += result[pX].total;
        }
        return result;
    },

  /************************************************************************
   * Organizer for All parsing and link to csv conver and master analysis *
   * @param  {Array} data  An array of JSON KeyStroke Data Files          *
   * @return {void}       void;                                           *
   ************************************************************************/

    speed: (data) => {
        var result = { 'words_per_minute': 0 };
        for (pX in data) {
            result[pX] = { 'words_per_minute': 0 }
            var keys = data[pX].KeyEvents;
            keys.forEach((val, index, arr) => {
                result[pX][pX + '_' + index + 'words'] = 0;
                val.forEach((v, i, a) => {
                    if (v.type == 'keydown' && v.which == 32) {
                        result[pX][pX + '_' + index + 'words'] += 1;
                    }
                });
            });
        }
        return result;
    },

  /************************************************************************
   * Organizer for All parsing and link to csv conver and master analysis *
   * @param  {Array} data  An array of JSON KeyStroke Data Files          *
   * @return {void}       void;                                           *
   ************************************************************************/

    overLap: (data) => {

    }
}